...
for i in range(0, len(dataframe)):
    if (math.isnan(dataframe.iloc[i]['Maths %']))==False:
        if dataframe.iloc[i]['Watch TV']==1:
            count_1=count_1+1
            marks_1=marks_1+dataframe.iloc[i]['Maths %']
        if dataframe.iloc[i]['Watch TV']==2:
            count_2=count_2+1
            marks_2=marks_2+dataframe.iloc[i]['Maths %']
        if dataframe.iloc[i]['Watch TV']==3:
            count_3=count_3+1
            marks_3=marks_3+dataframe.iloc[i]['Maths %']
        if dataframe.iloc[i]['Watch TV']==4:
            count_4=count_4+1
            marks_4=marks_4+dataframe.iloc[i]['Maths %']
        

print 'Average Percentage of Maths Marks of Watch TV 1 (Never)- ',((marks_1/count_1))
print 'Average Percentage of Maths Marks of Watch TV 2 (Once a month)- ',((marks_2/count_2))
print 'Average Percentage of Maths Marks of Watch TV 3 (Once a week)- ',((marks_3/count_3))
print 'Average Percentage of Maths Marks of Watch TV 4 (Every day)- ',((marks_4/count_4))
