...
for i in range(0, len(dataframe)):
    if (math.isnan(dataframe.iloc[i]['Maths %']))==False:
        if dataframe.iloc[i]['Solve Maths in groups']==1:
            count_1=count_1+1
            marks_1=marks_1+dataframe.iloc[i]['Maths %']
        if dataframe.iloc[i]['Solve Maths in groups']==2:
            count_2=count_2+1
            marks_2=marks_2+dataframe.iloc[i]['Maths %']
        if dataframe.iloc[i]['Solve Maths in groups']==3:
            count_3=count_3+1
            marks_3=marks_3+dataframe.iloc[i]['Maths %']
        

print 'Average Percentage of Maths Marks of Solve Maths in groups 1 (Disagree)- ',((marks_1/count_1))
print 'Average Percentage of Maths Marks of Solve Maths in groups 2 (Neither)- ',((marks_2/count_2))
print 'Average Percentage of Maths Marks of Solve Maths in groups 3 (Agree)- ',((marks_3/count_3))
